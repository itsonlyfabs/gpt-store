...
router.get('/documentation', [authMiddleware, adminMiddleware], async (req, res) => {
  const { data, error } = await supabaseAdmin.from('documentation').select('*').order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  res.json(data);
});

router.post('/documentation', [authMiddleware, adminMiddleware], async (req, res) => {
  const { title, subtitle, context } = req.body;
  const { data, error } = await supabaseAdmin.from('documentation').insert([{ title, subtitle, context }]).select();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data[0]);
});

router.put('/documentation/:id', [authMiddleware, adminMiddleware], async (req, res) => {
  const { id } = req.params;
  const { title, subtitle, context } = req.body;
  const { data, error } = await supabaseAdmin.from('documentation').update({ title, subtitle, context }).eq('id', id).select();
  if (error) return res.status(500).json({ error: error.message });
  res.json(data[0]);
});

router.delete('/documentation/:id', [authMiddleware, adminMiddleware], async (req, res) => {
  const { id } = req.params;
  const { error } = await supabaseAdmin.from('documentation').delete().eq('id', id);
  if (error) return res.status(500).json({ error: error.message });
